# command for trivia api
PGPASSWORD=password psql -U  postgres
\l to list the databases
\c to connect to any db
\dt to list tables
\d tablename to get the details

# to set update database with powershell script
createdb -U postgres
Password is password
Get-Content trivia.psql | psql.exe trivia  postgres

# manually insert multiple data to questions
